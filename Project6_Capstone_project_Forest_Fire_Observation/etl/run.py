...
class DeployPySparkScriptOnAws(object):
    ...
    def run(self):
        ...
        c = session.client('emr', region_name='us-west-2')                           # Open EMR connection
    ...
    def temp_bucket_exists(self, s3):
        ...
        """
        try:
            logger.info(s3.meta.client.head_bucket(Bucket=self.s3_bucket_temp_files))
                ...
                'InstanceGroups': [
                    {
                        ...
                        'Market': 'ON_DEMAND',
                        'InstanceRole': 'MASTER',
                        'InstanceType': self.emr_instance_type,
                        'InstanceCount': 1,
                        'Configurations': [
                          {
                    ...
                    },
                    {
                        ...
                        'InstanceRole': 'CORE',
                        'InstanceType': self.emr_instance_type,
                        'InstanceCount': self.emr_core_instance_count,
                        'Configurations': [
                          {
                             "Classification": "spark-env",
                             "Configurations": [
    ...
    def step_spark_submit(self, c):
        ...
        logger.info("Added step 'spark-submit' with argument '{}'".format('etl.py'))
...
if __name__ == "__main__":
    ...
    path_script = config.get('GENERAL','path_script')
    s3_bucket_logs = config.get('S3','s3_bucket_logs')
    s3_bucket_temp_files = config.get('S3','s3_bucket_temp_files')
    ...
    emr_core_instance_count = config.get('EMR','emr_core_instance_count')
    aws_access_key_id = config.get('AWS','key')
    aws_secret_access_key = config.get('AWS','secret')

    deployment = DeployPySparkScriptOnAws(
        app_name = app_name,
        user = user,
        ...
        emr_core_instance_count = int(emr_core_instance_count),
